'''
Current clamp demo using Allen Brain's model 472442377.
...
from neuron472442377 import Neuron472442377
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472442377(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472442377; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-93.3055648804)
        h.fcurrent()
